...
PRICE_IMAGE_URL = "https://drive.google.com/file/d/1tELToj0vtDrSb3-sh_nXTvqXlV4r7ayn/view?usp=sharing"  # Замість this-url на ваш реальний

async def set_bot_commands():
    commands = [
        BotCommand(command="start", description="Запустити бота"),
        BotCommand(command="broadcast", description="Розсилка повідомлень (тільки для адміністратора)"),
        BotCommand(command="price", description="Показати прайс")
    ]
...
async def start_handler(message: Message):
    ...
    if message.chat.id not in subscribers:
        ...
        # Сповіщення адміністратору про нову підписку
        await bot.send_message(ADMIN_ID, f"Нова підписка! ID користувача: {message.chat.id}")
    else:
...
async def show_price(message: Message):
    try:
        # Відправляємо фото прайсу за URL
        await message.answer_photo(PRICE_IMAGE_URL, caption="Ось наш прайс. Звертайтесь для запису на масаж!")
    except Exception as e:
        await message.answer("Виникла помилка при відправці прайсу. Спробуйте ще раз.")
        print(f"Помилка при відправці прайсу: {e}")
